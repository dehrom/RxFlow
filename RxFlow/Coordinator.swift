...
class FlowCoordinator: HasDisposeBag {
    ...
    init(for flow: Flow,
        ...
        identifier = "\(type(of: flow))-\(UUID().uuidString)"
    }
    ...
    func coordinate() {
        ...
        steps
            ...
            .map { [unowned self] (stepContext) -> (StepContext, NextFlowItems) in
                ...
                (stepContext, self.flow.navigate(to: stepContext.step))
                ...
                case let .multiple(flowItems):
                    return (stepContext, flowItems)
                case let .one(flowItem):
                    return (stepContext, [flowItem])
                case let .end(stepToSendToParentFlow):
                ...
                case let .triggerParentFlow(stepToSendToParentFlow):
            ...
            .filter { (nextFlowItem) -> Bool in
                ...
                // should be processed
                !(nextFlowItem.nextPresentable is Flow)
            }
        ...
        flow.rxDismissed.subscribe(onSuccess: { [weak self] in
...
public final class Coordinator: HasDisposeBag, Synchronizable {
    private var flowCoordinators = [String: FlowCoordinator]()
    fileprivate let loadingSubject = PublishSubject<Flow>()
    ...
    public func coordinate(flow: Flow, withStepper stepper: Stepper) {
        coordinate(flow: flow, withStepper: stepper, withParentFlowCoordinator: nil)
    }
...
extension Coordinator: FlowCoordinatorDelegate {
    func load(_ flow: Flow) {
        loadingSubject.onNext(flow)
    }

    func navigateToAnotherFlow(withParentFlowCoordinator parentFlowCoordinator: FlowCoordinator, withNextFlowItem nextFlowItem: NextFlowItem) {
        if let nextFlow = nextFlowItem.nextPresentable as? Flow {
            coordinate(flow: nextFlow, withStepper: nextFlowItem.nextStepper, withParentFlowCoordinator: parentFlowCoordinator)
        }
    }
    ...
    func willNavigate(to stepContext: StepContext) {
        if let withinFlow = stepContext.withinFlow,
            ...
            willNavigateSubject.onNext((withinFlow, stepContext.step))
        }
    }
    ...
    func didNavigate(to stepContext: StepContext) {
        if let withinFlow = stepContext.withinFlow,
            !(stepContext.step is NoneStep) {
            didNavigateSubject.onNext((withinFlow, stepContext.step))
        }
    }
}
